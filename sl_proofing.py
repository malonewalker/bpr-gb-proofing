...
def extract_words_trimmed(page, top_margin=20, bottom_margin=20):
    ...
    words = page.extract_words(use_text_flow=True, x_tolerance=2, y_tolerance=3, keep_blank_chars=False)
    if not words:
        return []
    page_h = page.height
    return [w for w in words if (w["top"] > top_margin and w["bottom"] < page_h - bottom_margin)]
...
def group_lines_by_y(words, y_tol=3.0):
    ...
    for w in words_sorted:
        if current_y is None or abs(w["top"] - current_y) <= y_tol:
            ...
            lines.append({"y": current[0]["top"], "x0": x0, "x1": x1, "text": text, "words": current[:]})
            current = [w]
            current_y = w["top"]
    if current:
        current.sort(key=lambda ww: ww["x0"])
...
def detect_two_profiles_split_y(words, page_height, band=0.08):
    ...
    near_mid = [w for w in words if (mid_y - page_height*band) <= w["top"] <= (mid_y + page_height*band)]
    above = [w for w in words if w["top"] < mid_y - page_height*band]
    below = [w for w in words if w["top"] > mid_y + page_height*band]
    if len(near_mid) <= max(2, 0.15 * max(len(above), len(below))) and len(above) > 10 and len(below) > 10:
...
def detect_gutter_x(words, bins=24):
    ...
    def bi(x):
        if x <= xmin: return 0
        if x >= xmax: return bins-1
        return min(bins-1, int((x - xmin) / width * bins))
    for x in xs: counts[bi(x)] += 1
    left_band = int(bins*0.30); right_band = int(bins*0.70)
    if right_band <= left_band: left_band, right_band = 1, bins-2
...
def find_published_name_left(lines_left):
    ...
    for ln in lines_left[:8]:
        t = ln["text"].strip()
        if t and "TABLE OF CONTENTS" not in t.upper() and "BEST PICK REPORTS" not in t.upper():
            return t, ln["y"]
    return "", None
...
def subsection_between(lines_text, start_kw, end_kws):
    ...
    start_idx = next((i for i, ln in enumerate(lines_text) if start_kw.lower() in ln.lower()), None)
    if start_idx is None:
        return ""
    end_idx = next((j for j in range(start_idx+1, len(lines_text))
                    if any(ek.lower() in lines_text[j].lower() for ek in end_kws)), len(lines_text))
        ...
        if "Best Pick Reports recommends:" in ln:
            for nxt in lines[i+1:]:
                if nxt.strip():
                    return nxt.strip()
    for ln in lines[:6]:
        if (re.search(r"[A-Z][a-z]+(?:\s+[&A-Za-z][A-Za-z]+)+", ln)
            and len(ln.strip()) <= 45
...
def _extract_between_markers(lines: List[str]) -> str:
    ...
    if start_idx is None:
        return ""
    end_idx = next((j for j in range(start_idx+1, len(lines)) if END_GLI_RE.search(lines[j])), None)
    if end_idx is None:
        end_idx = next((j for j in range(start_idx+1, len(lines)) if ANY_VERIFIED_RE.search(lines[j])), len(lines))
    payload = []
...
def parse_star_counts_right_margin(lines_all: List[dict], page_width: float) -> Dict[str, Optional[int]]:
    ...
    # bounds
    start_idx = next((i for i, ln in enumerate(lines_sorted)
                      if "homeowner satisfaction results" in ln["text"].lower()), None)
    if start_idx is None:
        return {}

    END_KWS = ["Rating:", "Verified", "Trade License", "Trade License(s) Not Required",
               "Best Pick Guaranteed", "Scan QR", "Services Offered",
               "Best Pick Reports recommends:", "Services Not Offered", "Minimum Job"]
    end_y = None
    ...
    right_side = [(y, x0, v) for (y, x0, v) in perline_candidates if x0 >= dynamic_thresh]
    if len(right_side) < 5:
...
def extract_profile_from_region(region_words, page, page_header_text: str):
    ...
    y_before = first_section_y if first_section_y is not None else (y_after + 60 if y_after else None)
    published_name_and_number = find_published_name_and_number(lines_all, page.width, y_after, y_before)
    ...
    ratings_block = subsection_between(
        joined_for_sections, "Homeowner Satisfaction Results",
        ["Rating:", "Verified", "Trade License", "Trade License(s) Not Required",
         "Best Pick Guaranteed", "Scan QR", "Services Offered",
         "Best Pick Reports recommends:", "Services Not Offered", "Minimum Job"]
    )
    ...
    if ratings_block:
        for ln in [l.strip() for l in ratings_block.splitlines() if l.strip()]:
            if reviews_total is None and re.search(r"(reviews\s*(surveyed|total|count)|total\s*reviews)", ln, re.I):
                m = INT_RE.search(ln)
                if m:
                    try:
                        ...
                        pass
            if recommendation_rate_pct is None and re.search(r"(recommend|would\s*recommend|recommendation\s*rate)", ln, re.I):
                m = PCT_RE.search(ln)
                if m:
                    try:
    ...
    verified_block, trade_license_numbers = _extract_verified_and_licenses(left_plain, right_plain)
    ...
    return {
        ...
        "Rating (out of 5)": extract_rating("\n".join(joined_for_sections)) if joined_for_sections else None,
...
def process_pdf(pdf_path: Path) -> pd.DataFrame:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_number, page in enumerate(pdf.pages[start_idx:], start=start_idx+1):
            ...
            regions = [(0, page.height)] if split_y is None else [(0, split_y), (split_y, page.height)]
    ...
    if "Recommendation Rate (%)" in df.columns:
        df["Recommendation Rate (%)"] = pd.to_numeric(df["Recommendation Rate (%)"], errors="coerce")
...
def pick_pdf_file_via_dialog() -> Path:
    ...
    if tk is None or filedialog is None:
        raise SystemExit("This script requires a GUI file dialog, but tkinter is unavailable.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract one row per half-page Company Profile from a Best Pick PDF (starting at TOC). Category equals page header."
    )
